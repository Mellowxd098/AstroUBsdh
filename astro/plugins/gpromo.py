...
#made by @danish_00 with sh1vam

from astro.utils import register, errors_handler
...
@astro.on(admin_cmd(pattern="gpromote ?(.*)"))
...
async def gben(astro):
    ...
    if astro.is_private:
        ...
        rank = astro.pattern_match.group(1)
    ...
    try:
        ...
        user, rank = await get_full_user(astro)
    ...
    if user:
        ...
        for x in telchanel:
          ...
          try:
             ...
             await astro.client(EditAdminRequest(x, user, rgt, rank))
...
@astro.on(admin_cmd(pattern="gdemote ?(.*)"))
