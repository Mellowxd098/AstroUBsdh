    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit(
                "**Something went Wrong\n Please report at @Astro_HelpChat", str(err)
            )
...
async def gben(userbot):
    ...
    if user:
        ...
        try:
            from astro.plugins.sql.gmute_sql import gmute
        except:
            pass
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"**Globally banned\nThis Nibba😂 Total Affected Chats **: `{a}`😏")
            except:
                b += 1
    else:
...
async def gunben(userbot):
    ...
    if user:
        ...
        try:
            from astro.plugins.sql.gmute_sql import ungmute
        except:
            pass
        try:
...
async def handler(rkG):
    if rkG.user_joined or rkG.user_added:
        try:
            from astro.plugins.sql.gmute_sql import is_gmuted
