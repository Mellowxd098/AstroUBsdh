...
@astro.on(admin_cmd(pattern=r"logic", outgoing=True))
@astro.on(sudo_cmd(pattern=r"logic"))
async def _(event):
    ...
    if x == 5:
        ...
        await eor(
            event, '`"Any video with “wait for it” in the title is simply too long."`'
        )
    ...
    if x == 11:
        ...
        await eor(event, '`"A folder is for things that you don\'t want to fold."`')
    ...
    if x == 20:
        ...
        await eor(event, '`"Technically, no one has ever been in an empty room."`')
    ...
    if x == 24:
        ...
        await eor(event, "`\"The word 'quiet' is often said very loud.\"`")
    ...
    if x == 45:
        ...
        await eor(event, '`"If Greenland actually turns green, we\'re all screwed."`')
    ...
    if x == 49:
        ...
        await eor(event, '`"Ask yourself why the the brain ignores the second the."`')
    ...
    if x == 53:
        ...
        await eor(event, '`"Raising a child is training your replacement."`')
    ...
    if x == 61:
        ...
        await eor(event, '`"6 to 6:30 feels more half-an-hour than 5:50 to 6:20"`')
    ...
    if x == 65:
        ...
        await eor(event, '`"A ton of whales is really only like half a whale."`')
    ...
    if x == 72:
        ...
        await eor(event, '`"The most effective alarm clock is a full bladder."`')

    if x == 73:

        await eor(
            event, '`"You probably just synchronized blinks with millions of people."`'
        )
    ...
    if x == 76:
        ...
        await eor(event, '`"We put paper in a folder to keep it from folding."`')

    if x == 77:

        await eor(
            event, '`"Somewhere, two best friends are meeting for the first time."`'
        )
    ...
    if x == 81:
        ...
        await eor(event, '`"Wikipedia is what the internet was meant to be."`')
    ...
    if x == 92:
        ...
        await eor(event, '`"Bubbles are to fish what rain is to humans."`')
    ...
    if x == 94:
        ...
        await eor(event, '`"A comma is a short pause, a coma is a long pause."`')

    if x == 95:

        await eor(event, '`"Someday you will either not wake up or not go to sleep."`')

    if x == 96:

        await eor(
            event, '`"Bermuda Triangle might be the exit portal of this simulation."`'
        )
